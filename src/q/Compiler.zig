...
fn compileNode(compiler: Compiler, node: Node.Index) Error!void {
    ...
    (switch (tree.nodeTag(node)) {
        ...
        .plus_colon => try compiler.emitByte(@intFromEnum(OpCode.flip)),
        ...
        .asterisk_colon => try compiler.emitByte(@intFromEnum(OpCode.first)),
        .percent => try compiler.emitByte(@intFromEnum(OpCode.divide)),
        .percent_colon => try compiler.emitByte(@intFromEnum(OpCode.reciprocal)),
        .ampersand => @panic("NYI"),
        ...
        .comma => try compiler.emitByte(@intFromEnum(OpCode.concat)),
        .comma_colon => try compiler.emitByte(@intFromEnum(OpCode.enlist)),
        .hash => @panic("NYI"),
        .hash_colon => @panic("NYI"),
        .underscore => @panic("NYI"),
        .underscore_colon => @panic("NYI"),
        .tilde => try compiler.emitByte(@intFromEnum(OpCode.match)),
        ...
        .at => try compiler.emitByte(@intFromEnum(OpCode.apply)),
        .at_colon => try compiler.emitByte(@intFromEnum(OpCode.type)),
        .dot => @panic("NYI"),
        .dot_colon => @panic("NYI"),
        ...
        .call => {
            ...
            switch (tree.nodeTag(func)) {
                ...
                .asterisk_colon => switch (args.len) {
                ...
                .comma_colon => switch (args.len) {
                    1 => try compiler.unary(.enlist, args[0]),
                    else => return error.Rank,
                },
                .tilde => switch (args.len) {
                    // 1 => try compiler.binary(.match, args[0], null),
                    2 => try compiler.binary(.match, args[0], args[1]),
                    else => return error.Rank,
                },
