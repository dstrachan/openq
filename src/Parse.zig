...
fn warnMsg(p: *Parse, msg: AstError) !void {
    @branchHint(.cold);
    switch (msg.tag) {
        .expected_expr,
...
fn fail(p: *Parse, tag: AstError.Tag) Error {
    @branchHint(.cold);
...
fn failMsg(p: *Parse, msg: AstError) Error {
    @branchHint(.cold);
    try p.warnMsg(msg);
...
pub fn parse(p: *Parse) !void {
    ...
    const root_exprs = try p.parseStatements();
...
fn expectVerb(p: *Parse, node: Node.Index, comptime sql_identifier: ?SqlIdentifier) !Node.Index {
    const verb = try p.parseVerb(node, sql_identifier);
...
fn parseSelect(p: *Parse) !Node.Index {
    const select_token = try p.assertToken(.keyword_select);
    ...
    if (!p.peekIdentifier(.{ .by = true, .from = true })) {
        while (true) {
            ...
            try p.scratch.append(p.gpa, expr);
            _ = try p.eatToken(.comma) orelse break;
        }
    }

    // By phrase
    const by_top = p.scratch.items.len;
    if (try p.eatIdentifier(.{ .by = true })) |_| {
        while (true) {
    ...
    if (try p.eatIdentifier(.{ .where = true })) |_| {
        while (true) {
...
fn parseExec(p: *Parse) !Node.Index {
    const exec_token = try p.assertToken(.keyword_exec);
...
fn parseUpdate(p: *Parse) !Node.Index {
    const update_token = try p.assertToken(.keyword_update);
...
fn parseDelete(p: *Parse) !Node.Index {
    const delete_token = try p.assertToken(.keyword_delete);
    ...
    if (try p.eatIdentifier(.{ .from = true })) |_| {
        // From phrase
        const from_expr = try p.expectExpr(.{ .where = true });
...
fn eatIdentifier(p: *Parse, comptime sql_identifier: SqlIdentifier) !?TokenIndex {
    return if (p.peekIdentifier(sql_identifier)) try p.nextToken() else null;
}

fn expectIdentifier(p: *Parse, comptime sql_identifier: SqlIdentifier) !TokenIndex {
    return try p.eatIdentifier(sql_identifier) orelse p.failMsg(.{
...
fn eatToken(p: *Parse, tag: Token.Tag) !?TokenIndex {
    return if (p.tokenTag(p.tok_i) == tag) try p.nextToken() else null;
}

fn assertToken(p: *Parse, tag: Token.Tag) !TokenIndex {
    assert(p.tokenTag(p.tok_i) == tag);
    return p.nextToken();
}
